...
const _now = Date.now();
const _t = (days: number): string =>
  new Date(_now - days * 24 * 60 * 60 * 1000).toISOString();
...
const MOCK_CONTEXT: string = `I'm Maya, CEO of a 4-person seed-stage AI startup. Right now I'm raising
...
export const server = setupServer(
  http.get("/api/emails", () => HttpResponse.json(MOCK_EMAILS)),
